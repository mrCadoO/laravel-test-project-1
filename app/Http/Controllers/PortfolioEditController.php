...
class PortfolioEditController extends Controller
{
    public function execute(Portfolio $portfolio, Request $request){
        ...
        if($request->isMethod('delete')){
            $portfolio->delete();
            return redirect('admin')->with('status', 'Объект страницы Портфолио был успешно удален. ');
        }
        ...
        if($request->isMethod('post')){
            ...
            if($portfolio->update()){
                return redirect('admin')->with('status', 'Объект страницы Портфолио был успешно обновлен. ');
            }
        }

        $old_data = $portfolio->toArray();


        if(view()->exists('admin.portfolio_edit')){
            $data = [
                    'title' => 'Редактирование Объектов Портфолио. ',
                    'data' => $old_data
                     ];
